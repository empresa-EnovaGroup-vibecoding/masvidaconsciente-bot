...
- ⛔ **Falta:** el token de GitHub NO tiene el permiso **`workflow`**, así que rechaza subir el archivo del flujo (probado incluso por SSH con deploy key de escritura → mismo bloqueo, es política de GitHub). Solo la dueña puede autorizarlo: `gh auth refresh -h github.com -s workflow`, o crear el archivo desde github.com. El `deploy.yml` quedó guardado en el scratchpad, listo para subir. Alternativa sin permiso: un "vigilante" (cron) en cada servidor que revisa GitHub y despliega. **Acceso:** la llave SSH `~/.ssh/masvida_vps` abre root en LOS DOS servidores.
...
**Pendientes:**
- 🟡 **Auto-deploy — CASI listo (falta 1 permiso de la dueña)**: construido y PROBADO vía **GitHub Actions → API de Coolify** (deploy encola en ambos servidores). Falta autorizar el permiso **`workflow`** de GitHub para poder subir el archivo del flujo. Ver punto 5 arriba. (El env/config sigue SIN sincronizarse: el auto-deploy mueve solo CÓDIGO.)
